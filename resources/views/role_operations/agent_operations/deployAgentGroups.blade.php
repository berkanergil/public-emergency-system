    ...
    <div class="col-sm-6">
        <ol class="breadcrumb float-sm-right">
            ...
            <li class="breadcrumb-item active">
                <a href="{{ route('deployAgentGroups') }}">{{ __('Form Agent Groups') }}</a>
            </li>
            </li>
            {{-- <li class="breadcrumb-item active">
                @if ($role == '1')
...
@section('statistic_content')
    ...
    <form action="{{ route('deployAgentGroups') }}" method="post">
        ...
        <div class="container bg-white rounded p-4">
            <div>
                <h3 class="my-2 text-bold create_staff_form"> {{ __('Agent Grouping Form') }}</h3>
                <hr class="create_staff_form">
            </div>
            <div class="row">
                <div class="col-lg-6 col-md-6 col-sm-12 col-xs-12">
                    <h6 class="create_staff_form text-bold">{{ __('Select Agent') }}</h6>
                    <div class="p-4" style="min-width:1000px">
                        <select class=" form-select form-select-lg mb-3 available_agents" required name="agents_id[]"
                            ...
                            @foreach ($staffObject->availableAgents() as $agent)
                                ...
                                @else
                                    <option value={{ $agent->id }}>
                                        {{ Str::title($agent->name . ' ' . $agent->surname) .' (' .Str::title($agent->department->tr) .' Departmanı)' }}
                                    </option>
                                @endif
                            @endforeach
                        </select>
